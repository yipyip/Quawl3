...
class Level(object):
    ...
    def __init__(self, level_map, commenter, conf, **items):
        ...
        self.coord_things = dict(list(zip((thing.position for thing in things), things)))
    ...
    def is_active(self, actor):
        ...
        return actor in iter(self.coord_actors.values())


    def filter_deads(self):

        deads = set(actor for actor in self.coord_actors.values() if actor.is_dead)
        coac = self.coord_actors
        self.coord_actors = dict((pos, act) for pos, act in coac.items()
                                 if act not in deads)
    ...
    def render(self, renderer):
        ...
        list(map(renderer.draw_item, iter(self.coord_things.values())))
        list(map(renderer.draw_item, iter(self.coord_actors.values())))



    def _pr_future_moves(self, future_moves):

        for pos, acts in list(future_moves.items()):
            print(pos, acts)
            for a in acts:
